...
module kanon_delaunay_mod
  ...
  use kanon_error_mod, only:&
    ...
    &ERROR_ALLOCATION,&
    &ERROR_DEALLOCATION
  use kanon_molecule_mod, only:&
    &molecule_t
  use kanon_tools_mod
  implicit none

  !>@note adapted from "table_delaunay.f90"
  !>      see: https://people.sc.fsu.edu/~jburkardt/f_src/table_delaunay/table_delaunay.html
contains
  
  !>@params[in] molecule type
  subroutine Get_delaunay_triangles( mol, wrong )
    type(molecule_t), intent(in) :: mol
    integer :: node_dim, node_num, ierr=0
    integer :: triangle_num, triangle_order
    character(len=:), allocatable :: element_filename
    integer,allocatable :: triangle_neighbor(:,:), triangle_node(:,:)
    real(kind=DP), allocatable :: node_xy(:,:)
    real(kind=DP) :: sumx, sumy, sumz
    integer :: k, m
    ...
    do k = 1, m
       sumx = sumx + dabs(mol%atoms(k)%xyz(1))
       sumy = sumy + dabs(mol%atoms(k)%xyz(2))
       sumz = sumz + abs(mol%atoms(k)%xyz(3))
    end do

    node_dim = 2
    node_num = m

    allocate ( node_xy(1:node_dim,1:node_num), stat=ierr )
    if (ierr /= 0) call error%MsgError( 1, ERROR_ALLOCATION )
  

    if ( dabs(sumx) <= 1e-5) then
       do k  = 1, m
          node_xy(1,k) = mol%atoms(k)%xyz(2)
          node_xy(2,k) = mol%atoms(k)%xyz(3)
       end do
    elseif( dabs(sumy) <= 1e-5 ) then
        do k  = 1, m
           node_xy(1,k) = mol%atoms(k)%xyz(1)
           node_xy(2,k) = mol%atoms(k)%xyz(3)
        end do
    elseif( dabs(sumz) <= 1e-5 ) then
        do k  = 1, m
           node_xy(1,k) = mol%atoms(k)%xyz(1)
           node_xy(2,k) = mol%atoms(k)%xyz(2)
        end do
    end if    

    triangle_order = 3

    allocate ( triangle_node(triangle_order,3*node_num), stat=ierr )
    if (ierr /= 0) call error%MsgError( 1, ERROR_ALLOCATION )
    allocate ( triangle_neighbor(triangle_order,3*node_num), stat=ierr )
    if (ierr /= 0) call error%MsgError( 1, ERROR_ALLOCATION )

    call dtris2 ( node_num, node_xy, triangle_num, triangle_node, &
                &triangle_neighbor )

    element_filename = file_triangle%name  

    call i4mat_write ( element_filename, triangle_order, triangle_num, &
                     &triangle_node )

    deallocate ( node_xy, stat=ierr )
    if (ierr /= 0) call error%MsgError( 1, ERROR_DEALLOCATION )
    deallocate ( triangle_node, stat=ierr )
    if (ierr /= 0) call error%MsgError( 1, ERROR_DEALLOCATION )
    deallocate ( triangle_neighbor, stat=ierr )
    if (ierr /= 0) call error%MsgError( 1, ERROR_DEALLOCATION )

    wrong = .false.

  end subroutine Get_delaunay_triangles
...
!  Write the data.
    ...
    write( output_unit, '(a,i10)') 'GRID ',n
    
    if ( n == 1 ) then 
        write( output_unit, '(a,3i2)') 'REF ', table(1:m,1)
    elseif( n > 1 ) then
          if(mod(n,2)==0) then
            write( output_unit, '(a,3i2)') 'REF ', table(1:m,1)
          else
            write( output_unit, '(a,3i2)') 'REF ', table(1:m,(n-1)/2)
          end if  
    end if   
!
    do j = 1, n
